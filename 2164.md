...
해석 및 접근 방식
-
해당 문제는 **가장 위에 있는 카드**만 가지고 작업을 하는 것이다.

즉 **List** 와 **Queue** 로 작업하기 좋은 문제다.

데이터의 앞부분을 삭제하고 옮기는 작업이니 삽입 삭제가 유리한 **List** 혹은 LIFO 로 이루어진 **Queue** 가 적합하다 생각했다.
...
위의 두개의 문법중 두번째인 카드를 맨뒤로 보내는 *list.push_back()* 문법은 해당 카드를 복사해서 뒤에 추가하는 방식으로 구조가 이루어져있기에,
...
temp 변수에 복사를 하고 해당 temp를 뒤로 보내는 방식을 통해 작업을했는데 해당부분을 거치지 않고도 작업할 수 있다는것을 알게되었다.
